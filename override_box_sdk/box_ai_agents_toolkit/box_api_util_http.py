...
def _do_request(box_client: BoxClient, url: str):
    ...
    try:
        ...
        resp = requests.get(url, headers={"Authorization": f"Bearer {access_token}"}, verify=False)  # Netskope issue: Override to disable SSL verification for all requests made by the SDK
